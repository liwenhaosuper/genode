...
namespace Genode
{
	...
	class M4if : Mmio
	{
		...
		public:
			...
			M4if(addr_t const base) : Mmio(base) {}

			void set_region(addr_t addr, size_t size)
			{
				write<Wm_reg0_ddr0_end::Addr>((addr+size) >> 12);
			...
			addr_t violation_addr() { return read<Wm_reg0_addr>(); }
	};
}
