...
struct Ipu : Genode::Mmio
{
	...
	class Cp_mem
	{
		public:
			...
			Cp_mem() { memset(this, 0, sizeof(Cp_mem)); }

			void * operator new(Genode::size_t size, void* addr) { return addr; }
	} __attribute__((packed));


	void init_dma_channel(unsigned channel,
	                      ...
	                      Genode::uint32_t stride, Genode::addr_t phys_base)
	{
		...
		memcpy(dst, (void*)&cpmem, sizeof(Cp_mem));
	}
	          ...
	          Genode::uint32_t stride, Genode::addr_t phys_base)
	{
		...
		/* set global alpha value to full transparency for overlay */
		write<Gr_wnd_ctl_sync>(0x80000000);
		write<Srm_pri2::Dp_m_srm>(Srm_pri2::Dp_m_srm::UPDATE_NOW);

		/* enable global alpha mode and overlay */
		write<Dp_com_conf>(0x5);
		write<Srm_pri2::Dp_m_srm>(Srm_pri2::Dp_m_srm::UPDATE_NOW);

		/* set overlay position to 0,0 */
		write<Dp_fg_pos_sync>(0);
		write<Srm_pri2::Dp_m_srm>(Srm_pri2::Dp_m_srm::UPDATE_NOW);
