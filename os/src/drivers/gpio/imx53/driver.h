...
class Imx53_driver : public Gpio::Driver
{
	private:
		...
		class Gpio_bank
		{
			...
			public:
				...
				{
					_reg.write<Gpio_reg::Int_mask::Pin>(enable ? 1 : 0, pin);
		...
		Imx53_driver()
		{
			for (unsigned i = 0; i < MAX_BANKS; ++i)
				for (unsigned j = 0; j < MAX_PINS; j++)
					_gpio_bank[i].regs()->write<Gpio_reg::Int_conf::Pin>(Gpio_reg::Int_conf::HIGH_LEVEL,j);
		}
	...
	public:
		...
		void direction(unsigned gpio, bool input)
		{
			...
			gpio_reg->write<Gpio_reg::Dir::Pin>(input ? 0 : 1,
			                                    _gpio_index(gpio));
		}
		...
		void write(unsigned gpio, bool level)
		{
			...
			gpio_reg->write<Gpio_reg::Data::Pin>(level ? 1 : 0,
			                                     _gpio_index(gpio));
		}
		...
		bool read(unsigned gpio)
		{
			...
			Gpio_reg *gpio_reg = _gpio_bank[_gpio_bank_index(gpio)].regs();
			return gpio_reg->read<Gpio_reg::Pad_stat::Pin>(_gpio_index(gpio));
		}
